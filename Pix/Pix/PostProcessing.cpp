...
void PostProcessing::DrawPixel(int x, int y)
{
    ...
    case EffectType::Mirror:
    {
        ...
        y = mRenderTarget.GetHeight() - y - 1.0f;
    }
    break;
    ...
    {
        int yRippleLenght = 45;
        color = mRenderTarget.GetColor(x, y);

        // Get a ratio from 0 - 1:
        float ratio = static_cast<float>(y % yRippleLenght) / static_cast<float>(yRippleLenght - 1);
        
        float circleValues = (ratio * X::Math::kTwoPi); // What part of circle are we on?
    ...
    case EffectType::FishEye:
    {
        ...
        const float effectDistance = 360.0f;
        const float halfWidth = static_cast<float>(mRenderTarget.GetWidth()) * 0.5;
        const float halfHeight = static_cast<float>(mRenderTarget.GetHeight()) * 0.5;

        X::Math::Vector2 rangeVec = { halfWidth, halfHeight };
        X::Math::Vector2 distanceVec = { x - halfWidth,y - halfHeight };
        ...
        distanceVec = rangeVec + (distanceVec * scale * ratio);
