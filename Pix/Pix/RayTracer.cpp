...
bool RayTracer::EndDraw()
{
    ...
    for (int y = 0; y < rows; ++y)
    {
        for (int x = 0; x < columns; ++x)
        {
            ...
            X::DrawPixel(x, y, Trace(primaryRay, 6));
        }
    }
...
X::Color RayTracer::Trace(const Ray& ray, int depth)
{
    ...
    else
    {
        ...
        Ray refractedRay;
        refractedRay.origin = closestPoint - (closestNormal * 0.001f);
        refractedRay.direction = ray.direction * eta + closestNormal * (eta * cosi - sqrt(k));
        refractedRay.direction = MathHelper::Normalize(refractedRay.direction);

        X::Color refractedColor = Trace(refractedRay, depth - 1);
